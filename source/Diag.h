...
/**
    ...
    A sign that never flips fails in ways an operator cannot tell apart from
    ...
    - **No audio reached the plugin** in Onset mode. Resolume fills an
      ...
      bin is zero, no onset ever fires, and the sign keeps its first picture
    ...
    so a sign that sits idle for an hour writes one line, not 180,000.
*/
namespace flipdot::diag
{
