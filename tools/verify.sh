...
#                 line by line, each line landing on the stated frame; Only
#                 Changes pulses exactly the discs that differ and Refresh All
#                 every disc; with no update the sign holds bit for bit; the
#                 stuck discs never move and there are exactly the stated
#                 fraction; the swing follows the stated torques to 1.1 px; a
#                 resize and a regrid keep every disc's side; loud audio on
#                 frame one fires nothing
#   software      every check again on Apple's software renderer
#                 (FDTEST_RENDERER=software), which is what a GPU-less CI
...
BUILD="${BUILD:-build}"
LOGS="${FLIPDOT_LOGS:-${TMPDIR:-/tmp}/flipdot-verify}"
mkdir -p "$LOGS"
failures=0
...
SHADERS_PY
	...
	if [ "$n" -lt 4 ]; then
		...
		# copy, means and board fragments. The extraction has gone stale.
...
rm -rf "$BUILD"
if cmake -B "$BUILD" -DCMAKE_BUILD_TYPE=Release >$LOGS/configure.log 2>&1 \
   && cmake --build "$BUILD" --parallel >$LOGS/build.log 2>&1; then
	pass "configured and built"
else
	fail "build failed -- see $LOGS/build.log"
	tail -25 $LOGS/build.log
	exit 1
fi
...
CHECKS="wipe changes bistable stuck rotation resize prime"
for t in $CHECKS; do
	log="$LOGS/$t.log"
	if "$FDTEST" "--$t" >"$log" 2>&1; then
		pass "fdtest --$t"
	else
		fail "fdtest --$t -- see $log"
		grep -E "FAIL" "$log" | head -6
	fi
done

#---------------------------------------------------------------------------
# The same checks on Apple's software renderer, asked for by renderer id.
# It is what a GPU-less CI runner falls back to, and it is where the board
# shader's own cos and sin were found to be out by 1e-3 (AGENTS.md).
#---------------------------------------------------------------------------
step "checks on the software renderer (FDTEST_RENDERER=software)"
for t in $CHECKS; do
	log="$LOGS/$t-software.log"
	if FDTEST_RENDERER=software "$FDTEST" "--$t" >"$log" 2>&1; then
		if grep -q "software renderer" "$log"; then
			pass "fdtest --$t (software)"
		else
			fail "fdtest --$t ran, but not on the software renderer -- see $log"
		fi
	else
		fail "fdtest --$t on the software renderer -- see $log"
		grep -E "FAIL" "$log" | head -6
	fi
done

step "negative controls (each broken model must fail its check)"
if "$FDTEST" --negative >$LOGS/negative.log 2>&1; then
	pass "$( tail -1 $LOGS/negative.log )"
else
	fail "a broken model passed its check -- see $LOGS/negative.log"
	grep -E "MISS" $LOGS/negative.log | head -6
fi

step "names"
if "$FDTEST" --names >$LOGS/names.log 2>&1; then
	pass "no name over 16 characters, none duplicated"
else
	fail "fdtest --names -- see $LOGS/names.log"
fi
...
#---------------------------------------------------------------------------
step "pipe"
bytes=$("$FDTEST" --pipe --size 64x36 --frames 3 </dev/null 2>/dev/null | wc -c | tr -d ' ')
if [ "$bytes" = "27648" ]; then
	pass "--pipe writes three whole 64x36 RGBA frames"
else
	fail "--pipe wrote $bytes bytes, expected 27648"
fi
"$FDTEST" --pipe --size 320x180 --frames 20 </dev/null 2>$LOGS/pipe.log | head -c 1 >/dev/null
...
for size in 640x360 320x180; do
	if python3 tools/sweep.py --binary "$FDTEST" --size $size --jobs 3 >$LOGS/sweep-$size.log 2>/dev/null; then
		pass "$size: $( tail -1 $LOGS/sweep-$size.log )"
	else
		fail "tools/sweep.py reports a dead control at $size -- see $LOGS/sweep-$size.log"
		tail -4 $LOGS/sweep-$size.log | sed 's/^/   /'
	fi
done

BUNDLE="$BUILD/Flipdot.bundle"
BIN="$BUNDLE/Contents/MacOS/Flipdot"
...
if [ "$(uname)" = "Darwin" ] && [ -d "$BUNDLE" ]; then
	...
	if codesign --force --sign - --timestamp=none "$tmp/Flipdot.bundle" >/dev/null 2>&1; then
	...
	if [ -x "$OXBOW" ]; then
		...
		case "$probe" in *"id:          FD01"*) pass "id is FD01" ;; *) fail "wrong FFGL id" ;; esac
		case "$probe" in *"name:        SW Flipdot"*) pass "name is SW Flipdot" ;; *) fail "wrong plugin name" ;; esac
